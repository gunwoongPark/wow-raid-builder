  ...
  "fantasy inline-flex items-center gap-1.5 transition-all duration-100 focus:outline-none focus:ring-2 focus:ring-amber-500/50 [border-image-repeat:stretch] border-solid",
  {
    ...
    variants: {
      ...
      variant: {
        ...
        outline: "border-amber-900/50 bg-black/40 text-amber-200/80 hover:bg-black/60",
        secondary: "text-slate-200 wc-btn-border [border-image-slice:16_fill]",
      },
    },
  }
)
...
function Badge({
  ...
  return (
    <Comp className={cn(badgeVariants({ faction, shape, size, variant }), className)} {...props}>
      {children}
    </Comp>
  )
}
